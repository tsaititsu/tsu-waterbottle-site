...
完整定義與判定規則以 `docs/TASK_PLAN_TEMPLATE.md` 為單一出處，本檔不重複維護另一份判定表。

規則：

* 開始修改、執行測試或操作 Git 前，必須先完成兩個標記。
* 若任務沒有標記，先停止並完成分類。
* 任務同時符合多個等級時，使用最高等級。
* 無法判斷時提高一級，不可自行降級。
* 任務範圍升高時，必須更新標記並重新檢查停止條件。
* SOL／Codex 等級不會授予額外權限，也不得放寬本專案或全域安全規則。
* SOL／Codex 等級與下方低風險／UI 風險／高風險的 PR 合併分類分開判定。
...
5. 建立 PR。
6. 標示 PR 合併風險。
7. 回報中已標示 SOL 模式與 Codex 任務等級。
8. 低風險依安心模式交由已設定的 auto-merge；UI 先等使用者確認 Preview；高風險不得 auto-merge。
