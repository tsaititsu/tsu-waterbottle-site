...
policy_facts as (
  select
    ...
    coalesce(
      (select role_names = array['public']::text[] from normalized_policy),
      false
    ) as roles_are_public,
    coalesce(
    ...
    coalesce(
      (
        select normalized_using_expression = 'auth.uid=idoris_admin'
        from normalized_policy
      ),
      false
    ) as using_is_expected_vulnerable_policy,
    coalesce(
      (
  ...
  select
    case
      when
        ...
        and acl_facts.authenticated_update
        and acl_facts.authenticated_is_admin_update
        ...
        and policy_facts.roles_are_public
        and policy_facts.using_exists
        and not policy_facts.with_check_exists
        and policy_facts.using_is_expected_vulnerable_policy
      then 'VULNERABLE_EXPECTED'
      when
        acl_facts.profiles_exists
        and acl_facts.rls_enabled
        and not acl_facts.anon_update
