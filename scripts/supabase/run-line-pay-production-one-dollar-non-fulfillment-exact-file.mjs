...
} from './validate-line-pay-production-one-dollar-non-fulfillment.mjs'
...
export function buildNonFulfillmentDeployAttestation(evidence, postflight) {
  return freeze({
    status: 'LINE_PAY_NT1_NON_FULFILLMENT_DEPLOYMENT_VALIDATED',
    ...
    database_postflight_attestation: {
      ...
      non_fulfillment_contract_ready: postflight.status === 'postflight_ready',
    },
  })
}

export const NON_FULFILLMENT_DATABASE_CONTRACT = Object.freeze({
  phaseFiles: Object.freeze({
    preflight: PREFLIGHT_FILE,
    deploy: DEPLOY_FILE,
  }),
  successMessages: Object.freeze({
    preflight: 'LINE_PAY_NT1_NON_FULFILLMENT_PREFLIGHT_VALIDATED',
    deploy: 'LINE_PAY_NT1_NON_FULFILLMENT_DEPLOYMENT_VALIDATED',
  }),
  ...
  parsePreflightOutput,
  parseDeployOutput,
  buildDeploySuccessAttestation: buildNonFulfillmentDeployAttestation,
})

export const NON_FULFILLMENT_VERIFICATION_CONTRACT = Object.freeze({
  phaseFiles: Object.freeze({ preflight: VERIFY_FILE }),
  successMessages: Object.freeze({
    preflight: 'LINE_PAY_NT1_NON_FULFILLMENT_POSTFLIGHT_VERIFIED',
  }),
  fixedFiles: Object.freeze([
    Object.freeze({ path: POSTFLIGHT_FILE, sha256: EXPECTED_POSTFLIGHT_SHA256 }),
    Object.freeze({ path: VERIFY_FILE, sha256: EXPECTED_VERIFY_SHA256 }),
  ]),
...
export function runNonFulfillmentDatabasePhase(phase, options = {}) {
  if (phase === 'verify') {
    return runFixedDatabasePhase(
      'preflight',
      NON_FULFILLMENT_VERIFICATION_CONTRACT,
      options,
    )
  }
  return runFixedDatabasePhase(
    phase,
    NON_FULFILLMENT_DATABASE_CONTRACT,
    options,
  )
}
...
async function main() {
  ...
  const result = await runNonFulfillmentDatabasePhase(process.argv[2])
