...
import {
  ...
  readAndValidateForensicFile,
  safeErrorCode,
  validateNodeVersion,
  validatePostgresImage,
  validateProductionChannel,
} from './validate-bank-transfer-production-forensic.mjs'
...
export const FORENSIC_CONTAINER_FILE = join(
...
export const FORENSIC_STATES = Object.freeze({
...
const FAILURE_CODE_PHASES = Object.freeze({
  FORENSIC_DOCKER_IMAGE_PULL_FAILED: Object.freeze(['docker_pull']),
  FORENSIC_TEMP_CREDENTIAL_CREATE_FAILED: Object.freeze([
    'credential_create',
  ]),
  FORENSIC_CONTAINER_START_FAILED: Object.freeze(['docker_run_start']),
  FORENSIC_CONTAINER_EXEC_FAILED: Object.freeze(['docker_run_start']),
  FORENSIC_DB_CONNECT_FAILED: Object.freeze(['psql_connection']),
  FORENSIC_SQL_EXECUTION_FAILED: Object.freeze(['forensic_sql']),
  FORENSIC_OUTPUT_INVALID: Object.freeze(['forensic_output']),
  FORENSIC_CAPTURE_LIMIT_EXCEEDED: Object.freeze([
    'docker_pull',
    'forensic_output',
  ]),
...
const STATE_FAILURES = Object.freeze({
  [FORENSIC_STATES.SOURCE_VALIDATED]: Object.freeze({
    code: 'FORENSIC_TEMP_CREDENTIAL_CREATE_FAILED',
    phase: 'credential_create',
  }),
  [FORENSIC_STATES.CREDENTIAL_CREATED]: Object.freeze({
    code: 'FORENSIC_DOCKER_IMAGE_PULL_FAILED',
    phase: 'docker_pull',
  }),
  [FORENSIC_STATES.IMAGE_PULL_STARTED]: Object.freeze({
    code: 'FORENSIC_DOCKER_IMAGE_PULL_FAILED',
    phase: 'docker_pull',
  }),
  [FORENSIC_STATES.IMAGE_PULL_COMPLETED]: Object.freeze({
    code: 'FORENSIC_CONTAINER_START_FAILED',
    phase: 'docker_run_start',
  }),
  [FORENSIC_STATES.CONTAINER_STARTED]: Object.freeze({
    code: 'FORENSIC_CONTAINER_EXEC_FAILED',
    phase: 'docker_run_start',
  }),
  [FORENSIC_STATES.PSQL_COMPLETED]: Object.freeze({
    code: 'FORENSIC_OUTPUT_INVALID',
    phase: 'forensic_output',
  }),
  [FORENSIC_STATES.OUTPUT_VALIDATED]: Object.freeze({
    code: 'TEMP_CREDENTIAL_CLEANUP_FAILED',
    phase: 'credential_cleanup',
  }),
  [FORENSIC_STATES.CREDENTIAL_CLEANED]: Object.freeze({
    code: 'TEMP_CREDENTIAL_CLEANUP_FAILED',
    phase: 'credential_cleanup',
  }),
})
...
class ForensicExecutionFailure extends Error {
  constructor(attestation) {
    super(attestation.failure_code)
    this.name = 'ForensicExecutionFailure'
...
export function validateFailureAttestation(attestation) {
  ...
  if (
    ...
    attestation.status !== 'FORENSIC_EXECUTION_FAILED' ||
    ...
    fail('FORENSIC_CONTAINER_EXEC_FAILED')
  }
  return true
}
...
export function classifyFailureForState(state, error) {
  ...
  if (!failure) fail('FORENSIC_CONTAINER_EXEC_FAILED')
  return {
    code: failure.code,
    phase: failure.phase,
  }
...
export function toSafeFailureAttestation(error) {
  if (
    error instanceof ForensicExecutionFailure &&
    error.attestation &&
...
export function isForensicExecutionFailure(error) {
  return error instanceof ForensicExecutionFailure
}
...
export function buildChildEnvironment(connection) {
  const environment = {
    ...
    PGAPPNAME: 'bank-transfer-production-read-only-forensic',
...
export function buildDockerRunArgs(connection, pgpassFile) {
  ...
  if (typeof pgpassFile !== 'string' || !isAbsolute(pgpassFile)) {
    fail('FORENSIC_CONTAINER_START_FAILED')
  }
...
export async function createCredentialFile(
  ...
  if (typeof runnerTemp !== 'string' || !isAbsolute(runnerTemp)) {
    fail('FORENSIC_TEMP_CREDENTIAL_CREATE_FAILED')
  }
  try {
  ...
  try {
    directory = await filesystem.mkdtemp(
      join(runnerTemp, 'bank-transfer-forensic-'),
    )
...
export async function runForensic({
  ...
  readAndValidateForensicFile(repositoryRoot)
  ...
  try {
    ...
    if (pullResult.captureExceeded) {
      failure = {
        code: 'FORENSIC_CAPTURE_LIMIT_EXCEEDED',
        phase: 'docker_pull',
      }
    ...
    if (result.code === 2) {
      failure = {
        code: 'FORENSIC_DB_CONNECT_FAILED',
    ...
    if (result.code === 3) {
      ...
      failure = {
        code: 'FORENSIC_SQL_EXECUTION_FAILED',
        phase: 'forensic_sql',
      }
      throw new Error(failure.code)
    }
  ...
  if (operationError) {
    throw new ForensicExecutionFailure(
...
if (invokedPath === import.meta.url) {
  main().catch((error) => {
    console.error(
      isForensicExecutionFailure(error)
