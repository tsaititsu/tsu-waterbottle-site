...
assert.match(pageSource, /fetch\(`\/api\/admin\/bookings\?limit=50&offset=\$\{offset\}`/)
...
assert.match(pageSource, /const requestSequenceRef = useRef\(0\)/)
assert.match(pageSource, /const requestSequence = \+\+requestSequenceRef\.current/)
assert.match(pageSource, /if \(requestSequence !== requestSequenceRef\.current\) return/)
assert.equal(pageSource.includes('唯讀管理頁'), true)
assert.equal(pageSource.includes('修改、取消與退款功能尚未開放'), true)
assert.equal(pageSource.includes('每批 50 筆載入遮蔽後預約摘要'), true)
assert.match(pageSource, /bookings\.length < totalBookings/)
assert.match(pageSource, /loadBookings\(bookings\.length\)/)
assert.equal(pageSource.includes('載入更多預約'), true)
...
assert.equal(pageSource.includes('tel:'), false)
