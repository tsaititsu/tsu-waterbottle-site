...
assert.equal(source.includes('出生地點'), true)
assert.equal(source.includes('例如：台灣彰化、美國紐約、大陸北京'), true)
assert.equal(source.includes('required'), true)
assert.equal(source.includes('birthPlace: birthPlace.trim()'), true)
assert.equal(source.includes('setBirthPlace(input.birthPlace'), true)
assert.equal(source.includes("setBirthPlace('')"), true)
assert.equal(source.includes('loadMemberAiChartProfiles'), true)
assert.equal(source.includes('saveMemberAiChartProfile'), true)
assert.equal(source.includes('deleteMemberAiChartProfile'), true)
assert.equal(source.includes("memberStorageStatus === 'ready'"), true)
assert.equal(source.includes('命盤會安全儲存在會員帳號'), true)
assert.equal(source.includes('const generateChart = async () => {'), true)
