...
assert.equal(divination.includes('getEllipticalFanTransform(index, 150, 78, isPicked ? 52 : 0)'), true)
assert.equal(divination.includes('getEllipticalFanTransform(index, 340, 130, isPicked ? 72 : 0)'), true)
assert.equal(divination.includes('Math.sin((angle * Math.PI) / 180) * radius'), true)
assert.equal(divination.includes('-Math.cos((angle * Math.PI) / 180) * yRadius + 65 - selectedLift'), true)
...
assert.equal(divination.includes('zIndex: index + 1'), true)
...
assert.equal(divination.includes('pendingIndex === null && hoveredCardIndex === index ? 220 : index + 1'), true)
assert.equal(divination.includes('pendingIndex === index ? 100'), false)
assert.equal(divination.includes('pendingIndex === index ? 300'), false)
