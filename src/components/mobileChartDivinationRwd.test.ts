...
assert.equal(divination.includes('w-[52px] min-w-0 shrink-0'), true)
assert.equal(divination.includes('mobileFanTransforms'), true)
assert.equal(divination.includes('mobile-card-fan-visual'), true)
assert.equal(divination.includes('style={{ transform: getMobileFanTransform(index) }}'), true)
assert.equal(divination.includes('translateY(28px) rotate(-12deg)'), true)
assert.equal(divination.includes('translateY(10px) rotate(-6deg)'), true)
assert.equal(divination.includes('translateY(0) rotate(0deg)'), true)
assert.equal(divination.includes('translateY(10px) rotate(6deg)'), true)
assert.equal(divination.includes('translateY(28px) rotate(12deg)'), true)
assert.equal(divination.includes('min-w-max'), false)
assert.equal(divination.includes('px-5 pb-8 pt-5'), true)
assert.equal(divination.includes('mobileFanTransforms.length - 1 ? "mr-2"'), true)
