...
assert.equal(divination.includes('const radius = 150'), true)
assert.equal(divination.includes('const yRadius = 78'), true)
assert.equal(divination.includes('Math.sin((angle * Math.PI) / 180) * radius'), true)
assert.equal(divination.includes('-Math.cos((angle * Math.PI) / 180) * yRadius + 65'), true)
...
assert.equal(divination.includes('style={{ transform: getMobileFanTransform(index), zIndex: index + 1 }}'), true)
