...
export default function AdminDetailSection({
  ...
  return (
    <section className="min-w-0 overflow-x-hidden rounded-2xl border border-borderSoft bg-white p-5 shadow-soft md:p-6">
...
export function AdminDetailField({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="min-w-0 rounded-xl bg-[#faf7ff] p-4">
      ...
      <dd className="mt-2 min-w-0 break-words text-sm leading-7 text-textDark [overflow-wrap:anywhere]">{value || '未提供'}</dd>
    </div>
  )
}
