...
assert.equal(hero.includes('text-[26px]'), true)
assert.equal(hero.includes('leading-[1.2]'), true)
assert.equal(hero.includes('<span className="block">紫微命盤分析 × 紫微牌卡</span>'), true)
assert.equal(hero.includes('whitespace-normal sm:hidden'), true)
assert.equal(hero.includes('hidden max-w-full whitespace-normal sm:block'), true)
