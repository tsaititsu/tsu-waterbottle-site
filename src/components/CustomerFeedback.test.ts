...
assert.equal(
  carouselSource.includes('const CONTINUOUS_SCROLL_SPEED_PX_PER_SECOND = 28'),
  true,
)
assert.equal(carouselSource.includes('const MANUAL_RESUME_DELAY_MS = 2500'), true)
assert.equal(carouselSource.includes('AUTOPLAY_INTERVAL_MS'), false)
assert.equal(carouselSource.includes('window.setTimeout'), false)
assert.equal(carouselSource.includes('window.requestAnimationFrame(animate)'), true)
assert.equal(carouselSource.includes('snap-mandatory'), false)
assert.equal(carouselSource.includes('continuousScrollPositionRef'), true)
assert.equal(carouselSource.includes('window.cancelAnimationFrame(animationFrameId)'), true)
assert.equal(carouselSource.includes('viewport.scrollLeft +='), true)
assert.equal(carouselSource.includes('elapsedMs / 1000'), true)
assert.equal(carouselSource.includes("window.matchMedia('(prefers-reduced-motion: reduce)')"), true)
assert.equal(carouselSource.includes('if (prefersReducedMotion) return'), true)
...
assert.equal(carouselSource.includes('onMouseEnter={() => setIsHoverPaused(true)}'), true)
assert.equal(carouselSource.includes('setIsHoverPaused(false)'), true)
assert.equal(carouselSource.includes('onFocusCapture={() => setIsFocusPaused(true)}'), true)
assert.equal(carouselSource.includes('onPointerDown={() => setIsPointerDown(true)}'), true)
assert.equal(carouselSource.includes("window.addEventListener('pointerup'"), true)
assert.equal(carouselSource.includes("window.addEventListener('pointercancel'"), true)
